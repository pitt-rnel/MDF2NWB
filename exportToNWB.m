...
% Find extension file path
[dir, ~, ~] = fileparts(which(mfilename));
extFile = fullfile(dir, 'spec', 'ndx-rnel-metadata.namespace.yaml');
if ~isfile(extFile)
    error('Extension file ''ndx-rnel-metadata.namespace.yaml'' does not exist')
end

disp('Generating extension classes...');

% Generate the extension classes
generateExtension(extFile);

disp('Exporting data...');

% Grant extension class
grantExt = types.ndx_rnel_metadata.GrantExtension( ...  
    'investigators', strjoin({grant.PI, grant.COI}, ', '), ...
    'url', grant.URL, ...
    'project_number', grant.projectNumber, ...
    'title', grant.officialName, ...
    'alt_names', strjoin([grant.alternativeNames, {grant.name}, {grant.description.name}], ', '), ...
    'acknowledgements', grant.description.acknowledgements, ...
    'additional_links', grant.description.additional_links, ...
    'funding', grant.description.funding, ...
    'start_date', grant.startDate, ... % TODO: Recommended yyyydate format
    'end_date', grant.endDate ... % TODO: Recommended date format
);
...
for i = 1:2
    subject = types.core.Subject( ...
        ...
        'date_of_birth', datetime(grant.subjects(1).dob), ...
        'weight', [num2str(grant.subjects(1).weight) ' ' grant.subjects(1).weight_units], ...
    ...
    nwb = NwbFile( ...
        ...
        'general', types.untyped.Set('grant', grantExt), ... % Lab specific meta data
